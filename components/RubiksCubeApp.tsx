...
export default function RubiksCubeApp() {
  ...
  return (
    <main
      className="min-h-screen bg-zinc-950 flex flex-col overflow-hidden select-none"
      style={{
        background:
          "radial-gradient(ellipse at 50% 0%, rgba(99,102,241,0.10) 0%, #09090b 55%)",
      }}
    >
      {/* Header */}
      <header className="flex items-center justify-between px-5 pt-5 pb-1 flex-shrink-0">
        <div>
      ...
      {/* Controls */}
      <div className="flex-shrink-0 px-4 pb-5 pt-1 space-y-3">
        {/* Scramble / Reset */}
        ...
        <div className="flex flex-wrap gap-1.5 justify-center max-w-sm mx-auto">
          {FACE_MOVES.map((move) => (
            <button
              ...
              disabled={!isReady}
              className="
                w-12 h-9 rounded-lg text-sm font-mono font-semibold
